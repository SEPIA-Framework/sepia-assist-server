...
import net.b07z.sepia.server.assist.chats.Question_how_are_you;
import net.b07z.sepia.server.assist.chats.Question_languages;
 ...
 *
 */
public class Chat_Preprocessor implements ApiInterface{
	
	//info
	public ApiInfo getInfo(String language){
		return new ApiInfo(Type.database, Content.data, true);
	}
		...
		//else link to a specific chat script if available or produce an answer
			...
			//questions
				...
				if (info.matches("how_are_you")){
					return Question_how_are_you.get(NLU_result);
				
				//languages?
				}else if (info.matches("languages")){
					return Question_languages.get(NLU_result);
