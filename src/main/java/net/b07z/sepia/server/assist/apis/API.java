...
public class API {
	...
	public String cmdSummary = "";				//command summary of this API request, needed to reconstruct it after client feedback is requested (and other stuff)
	...
	public int dialogStage = 0;					//dialog_stage as seen in NLU_Input (only set when modified inside API) send to client and client should send back
	
	//not (yet) in JSON result included:
	private NluResult nluResult;				//keep a reference to the NLU_Result for building API_Result
	 ...
	 */
	public API(NluResult nluResult){
		
		this(nluResult, new ApiInfo(Type.unspecified, Content.unspecified, true));
	}
	/**
	 * Default constructor for an API including API_Info. Some variables will be set to their proper values here. 
	 */
	public API(NluResult nluResult, ApiInfo apiInfo){
	 ...
	 */
	public ApiResult buildApiResult(){
		...
		ApiResult result = new ApiResult(status, answer, answerClean, htmlInfo, cardInfo, actionInfo, hasInfo, hasCard, hasAction);
