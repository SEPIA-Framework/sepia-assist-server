 ...
 *
 */
public class SoccerBundesligaInfo implements ApiInterface{
	...
	public static String getLocal(NSection section, String language){
		return News_RssFeeds.getLocal(section, language);
	}
	public static String getLocal(String term, String language){
		return News_RssFeeds.getLocal(term, language);
	}
	...
	public ApiResult getResult(NluResult nluResult){
		...
		if (!section.equals(NSection.soccer.name()) || !(type.equals(NType.results.name()) || type.equals(NType.table.name()))){
			...
			return new News_RssFeeds().getResult(nluResult);
		}
