 ...
 * question. Answers can be given in an answer_set. In case there is more than one answer (separated by "||") a random answer will be chosen.
 * 
 * @author Florian Quirin
 *
 */
public class Open_CustomLink {
	
	/**
	 * The default method to create an API_Result handling all the question/answer/action construction. 
	 * @param NLU_result - typically this is given by a direct command
	 * @return API_result
	 */
	public static ApiResult get(NluResult NLU_result){
		...
		//get parameters
		String url = NLU_result.getParameter("url");							//the url to call (can include wildcards in the form ***)
		
		String parameter_set = NLU_result.getParameter("parameter_set");		//the parameters filling the wildcards connected by "&&"
		String question_set = NLU_result.getParameter("question_set");			//a set of questions to the wildcards connected by "&&"
		String answer_set = NLU_result.getParameter("answer_set");				//a set of answers to complete the command, if there is more than one (separated by "||") a random one will be chosen
		
		String title = NLU_result.getParameter("title");					//title of link-card
		if (title.isEmpty()) title = "Link";
		
		String description = NLU_result.getParameter("description");		//description of link-card
		if (description.isEmpty()) description = ActionBuilder.getDefaultButtonText(api.language);
		
		String iconUrl = NLU_result.getParameter("icon_url");				//icon URL to be used for link-card
		...
		if (!parameter_set.matches("")){
			...
			if (nbr_of_openparams>0){
				...
				NLU_result.setParameter("parameter_set", parameter_set);
			}else{
				params = new String[]{""};
			}
		}
		...
		if (!question_set.matches("")){
			...
			NLU_result.setParameter("question_set", question_set);
		}
		...
		if ((nbr_of_openparams > 0) && (nbr_of_openparams <= questions.length)){
			...
			for (int i=0; i<params.length; i++){
				if (params[i].contains("***")){
					...
					return AskClient.question(question_key, "parameter_set", NLU_result);
				}
			}
		...
		//not enough questions for open parameters
			...
			for (int i=0; i<params.length; i++){
				if (params[i].matches("\\*\\*\\*")){
					return AskClient.question("default_ask_parameter_0a", "parameter_set", NLU_result);
				}
			}
		}
